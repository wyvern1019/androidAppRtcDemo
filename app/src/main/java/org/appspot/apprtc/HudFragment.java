...
public class HudFragment extends Fragment {
    ...
    private ImageView audioRecordStatView;
    private ImageView audioTrackStatView;

    private RealDoubleFFT transformer;
    private int blockSize = 256;
    private Canvas canvasAudioRecord;
    private Canvas canvasAudioTrack;
    private Paint paint;
    private Bitmap bitmapAudioRecord;
    private Bitmap bitmapAudioTrack;
    ...
    public View onCreateView(
        ...
        audioRecordStatView = controlView.findViewById(R.id.audio_record_stat);
        audioTrackStatView = controlView.findViewById(R.id.audio_track_stat);
        paint = new Paint();
        paint.setColor(Color.GREEN);
        bitmapAudioRecord = Bitmap.createBitmap(256, 100, Bitmap.Config.ARGB_8888);
        canvasAudioRecord = new Canvas(bitmapAudioRecord);
        audioRecordStatView.setImageBitmap(bitmapAudioRecord);
        bitmapAudioTrack = Bitmap.createBitmap(256, 100, Bitmap.Config.ARGB_8888);
        canvasAudioTrack = new Canvas(bitmapAudioTrack);
        audioTrackStatView.setImageBitmap(bitmapAudioTrack);
        return controlView;
    }
    ...
    public void updateAudioRecordData(double[] values) {
        canvasAudioRecord.drawColor(Color.BLACK);
        for (int i = 0; i < values.length; i++) {
            int x = i;
            int downy = (int) (100 - (values[i]) * 10);
            int upy = 100;
            canvasAudioRecord.drawLine(x, downy, x, upy, paint);
        }
        audioRecordStatView.invalidate();
    }

    public void updateAudioTrackData(double[] values) {
        canvasAudioTrack.drawColor(Color.BLACK);
        for (int i = 0; i < values.length; i++) {
            int x = i;
            int downy = (int) (100 - (values[i]) * 10);
            int upy = 100;
            canvasAudioTrack.drawLine(x, downy, x, upy, paint);
        }
        audioTrackStatView.invalidate();
    }
